...
def convert_file_tuple(file) -> TorrentFile:
...
class MetainfoFile:
    ...
    def from_path(cls, path: Path) -> "MetainfoFile":
        ...
        torrent_file._properties["info"] = external_torrent._struct.get("info") or dict()
    ...
    def __coerce_files(cls, files: Sequence) -> Sequence[TorrentFile]:
        return [convert_file_tuple(file) for file in files]
    ...
    def __validate_file_torrent_check(is_length_present: bool, is_files_present: bool):
        if is_length_present and is_files_present or not is_length_present and not is_files_present:
            raise ValueError('must contain either length key or files key, not both or neither')
